...
- **Details**: We avoid deep class-inheritance hierarchies. Within the tools (e.g. `yfinance_news.py` and `yfinance_ticker.py`), we explicitly separated side-effects (calling APIs) from pure data transformation functions (mapping fields and cleaning dictionaries), making the logic highly readable and easily unit-testable.
