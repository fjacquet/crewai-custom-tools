...
All notable changes to the `crew-custom-tools` project will be documented in this file.
...
- **Top-Level Exports**: Exposes `PerplexitySearchTool`, `YahooFinanceTickerInfoTool`, and `YahooFinanceNewsTool` directly from `crew_custom_tools`.
...
### Changed
- **Modular Packaging**: Renamed library package from `crewai-tools` to `crew-custom-tools` to prevent PyPI conflicts, updating all workspace files and plan structures.
