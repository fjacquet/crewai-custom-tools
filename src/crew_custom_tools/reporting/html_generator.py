...
class RenderReportToolSchema(BaseModel):
    ...
    sections: List[dict] = Field(..., description="A list of sections, each with 'heading' and 'content' keys.")
    images: Optional[List[dict]] = Field(default=None, description="Optional: list of image dicts with 'src', 'alt', 'caption'.")
    citations: Optional[List[str]] = Field(default=None, description="Optional: list of citation strings or URLs.")
    template_name: Optional[str] = Field(default="report_template.html", description="The template file to use (e.g. 'report_template.html', 'professional_report_template.html').")
...
class RenderReportTool(BaseTool):
    ...
    def __init__(self, template_dir: Optional[str] = None, **kwargs: Any):
        ...
        if not self._template_dir.exists():
            raise FileNotFoundError(f"HTML templates directory not found: {self._template_dir}")
    ...
    @api_tool(provider="Jinja2", endpoint="RenderReport", default_return="Error rendering HTML report.")
