...
class HtmlToPdfToolSchema(BaseModel):
    ...
    html_file_path: str = Field(..., description="The absolute or relative path to the input HTML file to be converted.")
    output_pdf_path: str = Field(..., description="The absolute or relative path where the generated PDF will be saved.")
...
class HtmlToPdfTool(BaseTool):
    ...
    @api_tool(provider="WeasyPrint", endpoint="HTMLToPDF", default_return="Error: PDF conversion failed.")
