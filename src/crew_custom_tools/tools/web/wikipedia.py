...
class WikipediaSearchToolInput(BaseModel):
    ...
    limit: Optional[int] = Field(default=5, description="The maximum number of results to return.")
...
class WikipediaArticleToolInput(BaseModel):
    ...
    action: ArticleAction = Field(default=ArticleAction.GET_SUMMARY, description="The action to perform on the article.")
...
class WikipediaArticleTool(BaseTool):
    ...
    description: str = (
        "Fetches content (summary, full article sections, etc.) from a specified Wikipedia article."
    )
    args_schema: type[BaseModel] = WikipediaArticleToolInput

    @api_tool(provider="Wikipedia", endpoint="ArticleFetcher", default_return="Error: Failed to fetch Wikipedia content.")
    def _run(self, title: str, action: ArticleAction = ArticleAction.GET_SUMMARY) -> str:
