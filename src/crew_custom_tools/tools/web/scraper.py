...
class UnifiedScraperTool(BaseTool):
    ...
    @api_tool(provider="UniversalScraper", endpoint="Scrape", default_return="Error: Scrape request failed.")
    def _run(self, url: str, provider: Optional[str] = None) -> str:
        ...
        try:
            ...
            return json.dumps({"success": False, "error": f"Scrape failed across all providers. Core error: {e}"})
