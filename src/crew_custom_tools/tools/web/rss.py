...
class RssFeedParserInput(BaseModel):
    ...
    days: int = Field(default=7, description="Number of past days of entries to return (default: 7).")
...
class OpmlParserInput(BaseModel):
    ...
    opml_file_path: str = Field(..., description="The absolute or relative path to the OPML file to be parsed.")


class OpmlParserTool(BaseTool):
    """A tool to parse OPML files to discover feed URLs."""
    name: str = "opml_parser"
    description: str = "Parses an OPML subscription file to extract standard RSS feed URLs."
