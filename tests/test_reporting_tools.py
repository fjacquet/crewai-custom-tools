...
def test_html_validator():
    ...
    valid_html = "<html><head><title>Test</title></head><body><h1>Hello</h1></body></html>"
...
def test_render_report_tool_success(mocker):
    ...
    mocker.patch("crew_custom_tools.reporting.html_generator.Environment", return_value=mock_env)
...
def test_html_to_pdf_conversion_success(mocker):
    ...
    mocker.patch("crew_custom_tools.reporting.pdf_generator.WEASYPRINT_AVAILABLE", True)
    
    # Mock WeasyPrint HTML class and write_pdf
    mock_html_class = mocker.MagicMock()
    mocker.patch("crew_custom_tools.reporting.pdf_generator.HTML", return_value=mock_html_class)
    ...
    tool = HtmlToPdfTool()
    result = tool._run(html_file_path="/path/to/report.html", output_pdf_path="/path/to/output.pdf")
...
def test_pestel_report_renderer(mocker):
    ...
    # Mock Jinja2 Template and Environment
    mock_template = mocker.MagicMock()
    mock_template.render.return_value = "<html><body><h1>PESTEL Analysis</h1></body></html>"
...
def test_financial_report_renderer(mocker):
    ...
    # Mock Jinja2 Template and Environment
    mock_template = mocker.MagicMock()
    mock_template.render.return_value = "<html><body><h1>Q2 Financial Report</h1></body></html>"
